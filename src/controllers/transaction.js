...
exports.addTransaction = async (req, res) => {
  try {
    // 1. prepare transaction data form body
    let data = req.body;
    data = {
    ...
    // 4. create snap API instance
    let snap = new midtransClient.Snap({
      isProduction: false,
      serverKey: process.env.MIDTRANS_SERVER_KEY,
    });

    // 5. set parameter for transaction
    let parameter = {
      transaction_details: {
        order_id: newData.id,
...
// get and set server and client key 
// from midtrans web configuration settings
// and save it on env file
